	...
	internal class MethodPatcherTools
	{
		...
		internal static bool PrefixAffectsOriginal(MethodInfo fix)
		{
			...
			return MethodPatcherTools.OriginalParameters(fix).Any(pair =>
			{
				...
				if (name == InjectedParameter.INSTANCE_PARAM)
					return false;
				if (name is InjectedParameter.ORIGINAL_METHOD_PARAM or InjectedParameter.ORIGINAL_PARAM)
					return false;
				if (name == InjectedParameter.STATE_VAR)
					return false;
