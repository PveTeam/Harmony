...
namespace HarmonyLib
{
	internal static class MethodCreatorTools
	{
		...
		internal const string FIELD_PREFIX = "__field_";
		...
		internal static bool AffectsOriginal(this MethodCreator creator, MethodInfo fix)
		{
			...
			return injectedParameters.Any(parameter =>
			{
				...
				if (parameter.injectionType == InjectionType.State)
					return false;
				if (parameter.injectionType == InjectionType.LocalState)
					return false;
		...
		)
		{
			...
			foreach (var injection in injections)
			{
				...
				if (injectionType == InjectionType.PrefixSkipped)
				{
					if (config.runOriginalVariable != null)
					{
						// invert runOriginal
				...
				if (paramRealName.StartsWith(INSTANCE_FIELD_PREFIX, StringComparison.Ordinal) || paramRealName.StartsWith(FIELD_PREFIX, StringComparison.Ordinal))
				{
					var fieldName = paramRealName.Substring(paramRealName.StartsWith(INSTANCE_FIELD_PREFIX, StringComparison.Ordinal) ? INSTANCE_FIELD_PREFIX.Length : FIELD_PREFIX.Length);
					FieldInfo fieldInfo;
				...
				if (injectionType == InjectionType.LocalState)
				{
					var ldlocCode = paramType.IsByRef ? OpCodes.Ldloca : OpCodes.Ldloc;
					if (config.localVariables.TryGetValue(InjectedParameter.LOCAL_STATE_VAR, out var stateVar))
						codes.Add(new CodeInstruction(ldlocCode, stateVar));
					else
						codes.Add(Ldnull);
					continue;
