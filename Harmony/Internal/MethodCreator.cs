...
namespace HarmonyLib
{
	internal class MethodCreator
	{
		...
		internal (MethodInfo, Dictionary<int, CodeInstruction>) CreateReplacement()
		{
			...
			var anyFixHasRunOriginal = config.AnyFixHas(InjectionType.RunOriginal) || config.AnyFixHas(InjectionType.PrefixSkipped);
			...
			config.WithFixes(fix =>
			{
				...
				if (declaringType is null)
					return;
				_ = config.localVariables.TryGetValue(InjectedParameter.LOCAL_STATE_VAR, out var maybeLocal);
				foreach (var injection in config.InjectionsFor(fix, InjectionType.LocalState))
				{
					var parameterType = injection.parameterInfo.ParameterType;
					var type = parameterType.IsByRef ? parameterType.GetElementType() : parameterType;
					if (maybeLocal != null)
					{
						if (!type.IsAssignableFrom(maybeLocal.LocalType))
						{
							var message = $"Local state variable of type {type} must be assignable from existing state variable of type {maybeLocal.LocalType}";
							throw new HarmonyException(message);
						}
						else
						{
					...
					var localStateVariable = config.DeclareLocal(type);
					config.AddLocal(InjectedParameter.LOCAL_STATE_VAR, localStateVariable);
					config.AddCodes(this.GenerateVariableInit(localStateVariable));
				}
			});
		...
		internal void AddPrefixes()
		{
			foreach (var fix in config.prefixes)
			{
				...
				if (MethodPatcherTools.OriginalParameters(fix).Any(pair => pair.realName == InjectedParameter.ARGS_ARRAY_VAR))
